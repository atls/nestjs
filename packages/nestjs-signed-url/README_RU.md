...
`@atls/nestjs-gcs-client`, `@atls/nestjs-s3-client` или провайдеру приложения.
...
- Для сервисов, которые отдают браузеру, мобильному клиенту или фоновому
  ...
  NestJS-приложение
- Для приложений на GCS, S3 или S3-совместимом хранилище, которым нужен один
  способ внедрять подпись ссылок в NestJS
...
- Позволяет доменным сервисам зависеть от `SignedUrlSigner`; для
  провайдерских параметров доступны `GcsSignedUrlSigner` и `S3SignedUrlSigner`
...
- Пакет не создаёт клиенты GCS или S3. Используйте
  `@atls/nestjs-gcs-client`, `@atls/nestjs-s3-client` или передайте уже
  настроенный клиент
