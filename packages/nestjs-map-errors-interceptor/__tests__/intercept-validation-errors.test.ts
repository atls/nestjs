...
import { createValidationFailedHandler } from './createValidationFailedHandler'
...
describe('MapValidationErrorsInterceptor', () => {
  ...
  describe('when validate against class with scalar property', () => {
    it('should map error', () => {
      class Foo { @IsString({ each: true }) bar: any }
      const obj = new Foo()
      obj.bar = 1
      ...
      return expect(
        interceptor.intercept(executionContext, createValidationFailedHandler(obj)).toPromise()
  ...
  describe('when validate against class with netsed object property', () => {
    it('should map error', () => {
      class NestedBar { @IsString({ each: true }) bar: any }
      class Foo { @ValidateNested() nestedBar: NestedBar }
  ...
  describe('when validate against class with arrary property', () => {
    it('should map error', () => {
      class NestedBar { @IsString({ each: true }) bar: any }
      class Foo { @ValidateNested({ each: true }) nestedBar: Array<NestedBar> }
